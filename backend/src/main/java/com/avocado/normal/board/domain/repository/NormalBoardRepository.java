...
public interface NormalBoardRepository extends JpaRepository<Item, Long> {
    ...
    // 전체 리스트 반환
    @Query("SELECT NEW com.avocado.normal.board.controller.dto.NormalResponseEntryDto(nh.autionId,i.id, i.name, nh.bidPrice, i.hopePrice, a.start_at, a.end_at) " +
            "FROM item i " +
            "JOIN aution a ON i.id = a.itemId " +
            "JOIN normalHistory nh ON a.id = nh.autionId")
    ...
    // 상세보기
    @Query("SELECT NEW com.avocado.normal.board.controller.dto.NormalItemDetailResponseDto(a.id, i.id, i.name, i.content, i.hopePrice, a.start_at, a.end_at) " +
            "FROM item i " +
            "JOIN aution a ON a.itemId = i.id " +
            "WHERE i.id = :id")
