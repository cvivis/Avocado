...
public class AuthService {
    ...
    public boolean validate(String requestAccessTokenInHeader) {
        ...
        return jwtTokenProvider.validateAccessTokenExpired(requestAccessToken); // true = 재발급
    }
    ...
    public TokenDto reissue(String requestAccessTokenInHeader, String requestRefreshToken) {
        ...
        String refreshTokenInRedis = redisService.getValue("RT(" + SERVER + "):" + principal);
        ...
        if(!jwtTokenProvider.validateRefreshToken(requestRefreshToken) || !refreshTokenInRedis.equals(requestRefreshToken)) {
            redisService.delete("RT(" + SERVER + "):" + principal); // 탈취 가능성 -> 삭제
            return null; // -> 재로그인 요청
        }
        ...
        // 토큰 재발급 및 Redis 업데이트
        redisService.delete("RT(" + SERVER + "):" + principal); // 기존 RT 삭제
    ...
    public TokenDto generateToken(String provider, String email, String authorities) {
        // RT가 이미 있을 경우
//        if(redisService.getValue("RT(" + provider + "):" + email) != null) {
//            redisService.delete("RT(" + provider + "):" + email);
//        }
...
//        saveRefreshToken(provider, email, tokenDto.getRefreshToken());
    ...
    public void saveRefreshToken(String provider, String principal, String refreshToken) {
        redisService.save("RT(" + provider + "):" + principal, // key
                refreshToken, // value
    ...
    // 로그아웃
    @Transactional
    public void signOut(String requestAccessTokenInHeader) {
        String requestAccessToken = resolveToken(requestAccessTokenInHeader);
        String principal = getPrincipal(requestAccessToken);
        ...
        if (refreshTokenInRedis != null) {
            redisService.delete("RT(" + SERVER + "):" + principal);
        }

        long expiration = jwtTokenProvider.getTokenExpirationTime(requestAccessToken) - new Date().getTime();
        redisService.save(requestAccessToken, "logout", expiration);
