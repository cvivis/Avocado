...
import { Box, Divider, HStack, Heading, Flex, Spacer, Button, Text } from "@chakra-ui/react";
...
function MyBidInfo(props) {
  ...
  const subscribe = () => {
    ...
    client.current.subscribe("/sub/normal/" + auctionId, (res) => {
      // server에게 메세지 받으면
      console.log("들어왔당.");
      ...
      setBidInfo((prevState) => {
        ...
        return {
          ...prevState,
          nowPrice: jsonBody.price,
          myPrice: jsonBody.price + BeforeNormalBid.setBidPlus(jsonBody.price),
          nowBidName: jsonBody.email,
        };
      });
    });
  };
  ...
  const publish = () => {
    ...
    client.current.publish({
      ...
      body: JSON.stringify({
        id: auctionId,
        price: bidInfo.myPrice,
        email: email,
        itemId: props.boardDetail.itemId,
      }),
  ...
  return (
    <Box>
      <Heading size={"2xl"}>{props.boardDetail.name}</Heading>
      ...
      <HStack>
        ...
        >
          {" "}
          {/* 여기에 입찰 기능 버튼 */}
          <Text textAlign={"left"}>입찰하기</Text>
          <Spacer />
          <Text textAlign={"right"}>{BeforeNormalBid.setBidUnit(bidInfo.myPrice)} 원</Text>
        </Button>
      </HStack>
    </Box>
  );
